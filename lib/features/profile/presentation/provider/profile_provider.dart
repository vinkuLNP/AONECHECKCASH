...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<void> deleteAccount(BuildContext context, AuthProvider auth) async {
    final id = await SessionManager.getClientRecordId();
    if (id == null) return;
    ...
    try {
      isLoading = true;
      notifyListeners();
      bool isDeleted = await deleteUser(id);
      if (isDeleted) {
        await auth.logout();
  ...
  Future<void> logout(BuildContext context, AuthProvider auth) async {
    try {
      isLoading = true;
      notifyListeners();
      await auth.logout();
